defmodule Ecto.Integration.PreloadTest do
  ...
  describe "bug in has_one-through scenario: preload not set when combining atom and existing join" do
    setup do
      post = TestRepo.insert!(%Post{})
      comment = TestRepo.insert!(%Comment{post: post})
      user = TestRepo.insert!(%User{})
      TestRepo.insert!(%Permalink{post: post, user: user})
    ...
    test "this works" do
      query =
        from(c in Comment,
          ...
          preload: [post_permalink: {pp, [user: u]}]
        )

      assert %{post_permalink: %{user: %User{}}} = TestRepo.one(query)
    end
    ...
    test "this fails" do
      query =
        from(c in Comment,
          join: pp in assoc(c, :post_permalink),
          join: u in assoc(pp, :user),
          preload: [post_permalink: {pp, [:post_comments_authors, user: u]}]
        )
